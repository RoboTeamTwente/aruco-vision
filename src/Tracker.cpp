...
Tracker::Tracker(int originID, int limitID) {
...
//std::vector<PosRotId> Tracker::performTracking(bool showDebug) {
//
//    std::vector<PosRotId> result = std::vector<PosRotId>();
//
//    cv::Ptr<cv::aruco::DetectorParameters> detectorParams = cv::aruco::DetectorParameters::create();
//    // Dictionary for the used markers.
...
//                                             translationVectors);
//	//Used to calculate the average resolution of each side of the limit marker.
//	double sum_resolution = 0;
//	int sum_counter = 0;
//
//        // Detect the origin and limit markers
//        if (!markerIds.empty()) {
//            for (int i = 0; i < markerIds.size(); i++) {
//                if (markerIds[i] == originMarker) {
//                    origin = markerCorners[i][0];
//		    sum_resolution += getAvgRes(markerCorners[i]);
//		    sum_counter++;
//                    cv::Rodrigues(rotationVectors[i], originRot);
//                } else if (markerIds[i] == limitMarker) {
//                    limit = markerCorners[i][0];
//		    sum_resolution += getAvgRes(markerCorners[i]);
//		    sum_counter++;
//                    cv::Rodrigues(rotationVectors[i], limitRot);
//                }
//            }
//	    if(sum_counter > 0){
//		//calculate the resolution per centimeter
//		double marker_res = (sum_resolution/sum_counter);
//		//width of the x value
//		double width = std::abs(limit.x - origin.x);
//		double heigth = std::abs(limit.y - origin.y);
//		grid_width = std::round((width/marker_res) * SURFBOTTRACKING_MARKER_SIZE);
//		grid_height = std::round((heigth/marker_res) * SURFBOTTRACKING_MARKER_SIZE);
//		if (showDebug) {
//			std::cout << "play area: width= " << grid_width <<
//					", height= " << grid_height << std::endl;
//		}
//	    }
//        }
//
...
std::vector<PosRotId> Tracker::performTrackingOnImage(cv::Mat image, bool showDebug) {
...
cv::Point2f
Tracker::calculatePosition(cv::Point2f origin, cv::Point2f limit, std::vector<cv::Point2f> observedPos, double angleZ) {
...
void Tracker::readCameraParameters(cv::Ptr<cv::Mat> cameraMatrix, cv::Ptr<cv::Mat> distCoeffs) {
...
double Tracker::euclideanDist(cv::Point2f a, cv::Point2f b) {
	cv::Point2f diff = a - b;
	return cv::sqrt(diff.x*diff.x + diff.y*diff.y);
}

double Tracker::getAvgRes(std::vector<cv::Point2f> corners) {
	double res = 0.0;
